...
do
    ...
    sqlcmd="CREATE TABLE weather$city (id STRING, ts TIMESTAMP, lon FLOAT, lat FLOAT, temp FLOAT, pressure INT, humidity INT, visibility INT, windspeed FLOAT, winddeg INT, measured INT) STORED AS ORC LOCATION 'hdfs://namenode:8020/user/nifi/weather$city';"
    echo "EXECUTING COMMAND \"$sqlcmd\""
    beeline -u jdbc:hive2:// -e "$sqlcmd"
done
